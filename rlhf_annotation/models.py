...
class AnnotationTask(db.Model):
  ...
  # 已经标注的样本数量
  annotated_sample_num = db.Column(db.Integer,nullable=False,defult=0)
  # 正在标注的数量
  in_progress_sample_num = db.Column(db.Integer,nullable=False,defult=0)
  ...
  #
  task_type = db.Column(db.Text,nullable=False)  # 任务类型
  task_status = db.Column(db.Text,nullable=False)  # 任务状态
